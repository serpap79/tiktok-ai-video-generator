#!/usr/bin/env python
"""Prepara el vídeo base del presentador: una sola vez por clip generado.

El presentador anterior era síntesis pura: un retrato fijo que `render/
presenter.py` hacía parpadear y girar cuadro a cuadro. Funcionaba, con el límite
declarado anteriormente: silueta, no persona. Ahora existe un clip fotorrealista de
THEO (Hailuo, identidad fijada en `brand.json`) en el que el parpadeo, el balanceo
de cabeza y la respiración **ya son humanos**: los genera un modelo de
vídeo, no numpy. Lo que este clip no tiene es una boca: escucha labios
cerrados desde el primer hasta el último cuadro (141, ninguno con los
labios separados). Es la división adecuada porque la boca es la
única pieza que depende de la narración y, por tanto, la única que no puede llegar lista.

Este script transforma el clip original en dos artefactos versionados:

- `<id>_base.mp4`: recorte del presentador en dos pistas de vídeo, color
  premultiplicado y máscara en escala de grises. `presenter.
  encode_command` ya usa el mismo formato: RGBA sin pérdida no cabe en disco y
  el RGB normal deja una franja oscura en el borde de la silueta.
- `<id>_base.json`: los puntos del rostro **en cada cuadro**. Sin ellos, la
  narración sintetizada no sabe adónde va la boca: la cabeza se mueve en el clip y una boca
  en coordenadas fijas se despega del rostro al primer balanceo.

Está fuera del paquete porque `rembg` y `mediapipe` suman más de 400 MB de
dependencias con onnxruntime y opencv. El agente se ejecuta tres veces al día y nunca necesita
esto; misma decisión que en `make_presenter_cutouts.py`.
...
Si el clip es corto, `render/presenter_video.py` aplica vaivén para cubrir
la narración; nada de este proceso depende de la duración.
"""
...
DEGRADE = 0.20
ALFA_MIN = 24
# Mediana temporal del alfa. El matting opera cuadro a cuadro y no sabe que es un
...
def ler_quadros(video: Path) -> tuple[list[Image.Image], float]:
    """Decodifica el clip completo en memoria. Son segundos, no minutos."""
    ...
    if sonda.returncode != 0:
        # ffprobe puede no existir junto al binario estático de imageio.
...
def _sonda_por_ffmpeg(video: Path) -> tuple[int, int, float]:
    ...
    if not m or not f:
        raise SystemExit(f"no se pudo analizar {video}")
...
def medir_rosto(quadros: list[Image.Image], fps: float) -> list[dict[str, tuple[float, float]]]:
    ...
    El modo VIDEO no es arbitrario: en modo IMAGE cada cuadro se detecta desde cero
...
def linha_da_marca(quadros: list[Image.Image]) -> int | None:
    ...
    El presentador respira, parpadea y se balancea; la marca no cambia un píxel en 15 s.
    ...
    Devuelve None cuando no hay marcas: un clip de una cuenta de pago no las tiene y el corte
    no debe acortar el busto sin motivo.
    """
    if len(quadros) < 4:
        return None
...
def processar(pid: str, video: Path) -> dict:
    ...
    if proc.wait() != 0:
        raise SystemExit(f"ffmpeg falló: {erro[-500:]}")
    ...
    meta = {
        ...
        # Un vector por punto, un par por cuadro. Resulta legible en el diff y no
...
def _encode(out: Path, w: int, h: int, fps: float) -> list[str]:
    """Color premultiplicado + máscara en escala de grises, ambos en un solo archivo."""
...
def main() -> int:
    ...
    if not video.exists():
        raise SystemExit(f"{video} no existe")
